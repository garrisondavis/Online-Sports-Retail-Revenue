...
This SQL project uses techniques such as aggregration, cleaning, labeling, Common Table Expressions, and correlation to produce recommendations on how a online sports store could maximize revenue. 
...
Select Count(*) As total_rows, -- Count all columns as total_rows
Count(i.description) as count_description, -- Count the number of non-missing entries for description, listing_price, and last_visited
...
Select b.brand, f.listing_price::int, count(*) -- Select the brand, listing_price as an integer, and a count of all products in finance 
From brands as b
Inner Join finance as f -- Join brands to finance on product_id
On b.product_id = f.product_id
Where f.listing_price > 0 -- Filter for products with a listing_price more than zero
Group By b.brand, f.listing_price -- Aggregate results by brand and listing_price, and sort the results by listing_price in descending order
...
Select b.brand, count(*), Sum(f.revenue) as total_revenue, -- Select the brand, a count of all products in the finance table, and total revenue
CASE WHEN f.listing_price < 42 Then 'Budget' -- Create four labels for products based on their price range, aliasing as price_category
When f.listing_price >= 42 And f.listing_price < 74 Then 'Average' -- Create four labels for products based on their price range, aliasing as price_category
When f.listing_price >= 74 And f.listing_price < 129 Then 'Expensive'
When f.listing_price >= 129 Then 'Elite' End As price_category
From brands as b
Inner Join finance as f -- Join brands to finance on product_id and filter out products missing a value for brand
On b.product_id = f.product_id
Where b.brand is not null
Group By b.brand, price_category -- Group results by brand and price_category, sort by total_revenue
...
%%sql
postgresql:///sports
Select brand, AVG(discount)*100 as average_discount -- Select brand and average_discount as a percentage
From brands as b
Inner Join finance as f -- Join brands to finance on product_id
On b.product_id = f.product_id
Group By b.brand -- Aggregate by brand
Having b.brand is not null -- Filter for products without missing values for brand
```
...
Select corr(r.reviews, f.revenue) as review_revenue_corr -- Calculate the correlation between reviews and revenue as review_revenue_corr
From reviews as r
Inner Join finance as f -- Join the reviews and finance tables on product_id
...
Select trunc(length(description), -2) as description_length, -- Calculate description_length
Round(AVG(rating::numeric), 2) as average_rating -- Convert rating to a numeric data type and calculate average_rating
From info as i
Inner Join reviews as r -- Join info to reviews on product_id and group the results by description_length
On i.product_id = r.product_id
Where description is not null
Group By description_length -- Filter for products without missing values for description, and sort results by description_length
...
Select b.brand, Extract(Month From t.last_visited) as month, count(*) as num_reviews -- Select brand, month from last_visited, and a count of all products in reviews aliased as num_reviews
From traffic as t
Inner Join reviews as r -- Join traffic with reviews and brands on product_id
On t.product_id = r.product_id
Inner Join brands as b
On t.product_id = b.product_id
Group By b.brand, month -- Group by brand and month, filtering out missing values for brand and month
Having b.brand Is Not Null and Extract(Month From t.last_visited) is not null
Order By b.brand, month; -- Order the results by brand and month
...
With footwear As ( -- Create the footwear CTE, containing description and revenue
    Select i.description, f.revenue
            From info as i
                  Inner Join finance as f
                  On i.product_id = f.product_id
                  Where (i.description ILike '%shoe%' OR i.description ILike '%trainer%' OR i.description ILike '%foot%') -- Filter footwear for products with a description containing %shoe%, %trainer, or %foot%
                  And i.description Is Not Null ) -- Also filter for products that are not missing values for description
Select count(*) as num_footwear_products, percentile_disc(.50) Within Group (Order By revenue) as median_footwear_revenue -- Calculate the number of products and median revenue for footwear products
From footwear; 
```
...
With footwear As ( -- Create the footwear CTE, containing description and revenue
    Select i.description, f.revenue
            From info as i
                  ...
                  On i.product_id = f.product_id
                  Where (i.description ILike '%shoe%' OR i.description ILike '%trainer%' OR i.description ILike '%foot%') -- Filter footwear for products with a description containing %shoe%, %trainer, or %foot%
                  And i.description Is Not Null ) -- Copy the footwear CTE from the previous task
Select count(*) as num_clothing_products, percentile_cont(.50) Within Group (Order By f.revenue) as median_clothing_revenue -- Calculate the number of products in info and median revenue from finance
From info as i
Inner Join finance as f -- Inner join info with finance on product_id
On i.product_id = f.product_id
Where i.description Not In (
                        Select description
                            From footwear); -- Filter the selection for products with a description not in footwear
